...
// Campo de amplitude que aceita VÍRGULA ou ponto (teclado BR).
// Mantém o texto local enquanto digita e converte para número ao confirmar.
const AmpInput = ({ value, onCommit, placeholder, className }) => {
  const [txt, setTxt] = React.useState(value != null && value !== 0 ? String(value).replace('.', ',') : '');
  React.useEffect(() => {
    // Sincroniza se o valor externo mudar e não estivermos no meio de uma edição
    const numLocal = parseFloat((txt || '').replace(',', '.'));
    if (value !== numLocal && !(isNaN(numLocal) && (value == null || value === 0))) {
      setTxt(value != null && value !== 0 ? String(value).replace('.', ',') : '');
    }
  }, [value]);
  const handle = (raw) => {
    // Aceita dígitos, vírgula e ponto; normaliza para o estado local
...
const ProgramRow = ({ label, prog, onChange, onRemove, tipoEletrodo, modoAmplitude, canRemove }) => {
  ...
  return (
    <div className="flex items-end gap-1 flex-wrap">
      ...
      {/* Amp / PW / Freq */}
      <AmpInput value={prog.amp} placeholder={modoAmplitude||'mA'}
        onCommit={(v) => onChange({ ...prog, amp: v })} className={paramCls}/>
...
export const ProgrammingEditor = ({
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      {/* Groups — vertical single column (works for 4-ring, 8-ring, directional) */}
      <div className="flex flex-col gap-2">
        {grupos.map(g => (
